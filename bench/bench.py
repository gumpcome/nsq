...
def _bootstrap(addr):
    ...
    for cmd in [
            ...
            'cd go/src/github.com/nsqio && git clone https://github.com/gumpcome/nsq',
            'cd go/src/github.com/gumpcome/nsq && git checkout %s' % commit,
            'sudo -S curl -s -o /usr/local/bin/gpm \
                https://raw.githubusercontent.com/pote/gpm/v1.2.3/bin/gpm',
            'sudo -S chmod +x /usr/local/bin/gpm',
            'cd go/src/github.com/gumpcome/nsq && \
                GOPATH=/home/ubuntu/go PATH=$PATH:/usr/local/go/bin gpm install',
            'cd go/src/github.com/gumpcome/nsq/apps/nsqd && \
                GOPATH=/home/ubuntu/go /usr/local/go/bin/go build',
            'cd go/src/github.com/gumpcome/nsq/bench/bench_writer && \
                GOPATH=/home/ubuntu/go /usr/local/go/bin/go build',
            'cd go/src/github.com/gumpcome/nsq/bench/bench_reader && \
                GOPATH=/home/ubuntu/go /usr/local/go/bin/go build',
    ...
    for id, addr in nsqd_hosts:
        try:
            ...
            for cmd in [
                    ...
                    'GOMAXPROCS=32 ./go/src/github.com/gumpcome/nsq/apps/nsqd/nsqd \
    ...
    for id, addr in producer_hosts:
        for nsqd_tcp_addr in nsqd_tcp_addrs:
            ...
            try:
                ...
                for cmd in [
                        'GOMAXPROCS=2 \
                            ./go/src/github.com/gumpcome/nsq/bench/bench_writer/bench_writer \
        ...
        for id, addr in consumer_hosts:
            for nsqd_tcp_addr in nsqd_tcp_addrs:
                ...
                try:
                    ...
                    for cmd in [
                            'GOMAXPROCS=8 \
                                ./go/src/github.com/gumpcome/nsq/bench/bench_reader/bench_reader \
                                --topic=%s --nsqd-tcp-address=%s:4150 --deadline=\'%s\' --size=%d \
                                --rdy=%d' % (
                                topic, nsqd_tcp_addr, deadline.strftime('%Y-%m-%d %H:%M:%S'),
